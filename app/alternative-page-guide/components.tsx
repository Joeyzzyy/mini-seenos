...
export function KeyTakeaways({ items, title = "Key Takeaways" }: { items: string[]; title?: string }) {
  return (
    <div className="mb-12 p-6 bg-gradient-to-br from-[#9A8FEA]/10 to-[#65B4FF]/5 border border-[#9A8FEA]/30 rounded-xl">
      <h2 className="text-lg font-semibold mb-4 text-white flex items-center gap-2">
        <svg className="w-5 h-5 text-[#9A8FEA]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function InternalLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link 
      href={href} 
      className="text-[#9A8FEA] hover:text-[#65B4FF] underline decoration-[#9A8FEA]/30 hover:decoration-[#65B4FF]/50 transition-colors"
    >
      {children}
    </Link>
  );
  ...
  return (
    <div className="mb-8">
      ...
      >
        ...
        Back to Alternative Page Guide (Pillar)
      </Link>
    </div>
  );
}
...
export function RelatedLinkCard({ 
  ...
  return (
    <Link 
      href={href}
      className="block p-4 my-6 bg-white/5 border border-white/10 rounded-lg hover:border-[#9A8FEA]/50 transition-all group"
    >
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-[#9A8FEA]/20 flex items-center justify-center shrink-0">
          <svg className="w-5 h-5 text-[#9A8FEA]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6"/>
          </svg>
        </div>
        <div>
          <h4 className="text-white font-medium group-hover:text-[#9A8FEA] transition-colors">{title}</h4>
        ...
        <li>
          <Link 
            ...
            className={`text-sm flex items-center gap-2 py-1 ${
              currentSlug === 'index' 
                ? 'text-[#9A8FEA] font-medium' 
                : 'text-gray-400 hover:text-white transition-colors'
            }`}
          >
            <span className="w-1.5 h-1.5 rounded-full bg-[#FFAF40]"></span>
            What is an Alternative Page Generator? (Pillar)
          </Link>
        </li>
        {pages.map((page, idx) => (
          <li key={idx}>
            <Link 
              href={`/alternative-page-guide/${page.slug}`}
              className={`text-sm flex items-center gap-2 py-1 ${
...
export function StatHighlight({ 
  ...
  return (
    <div className="my-6 p-6 bg-gradient-to-r from-[#FFAF40]/10 to-[#9A8FEA]/10 border-l-4 border-[#FFAF40] rounded-r-xl">
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0A0A0A]/80 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          <img src="/new-logo.png" alt="seopages.pro" className="h-8 w-auto" />
          <span className="text-white text-lg italic tracking-wide" style={{ fontFamily: '"Playfair Display", Georgia, serif' }}>
            seopages<span className="text-[#9A8FEA]">.</span>pro
          </span>
        </Link>
        <div className="flex items-center gap-4">
          <Link href="/alternative-page-guide" className="text-sm text-gray-400 hover:text-white transition-colors hidden sm:block">
            Guide
          </Link>
          <Link href="/#features" className="text-sm text-gray-400 hover:text-white transition-colors hidden sm:block">
            Features
          </Link>
          ...
          <Link 
            href="/projects" 
            className="px-4 py-2 bg-gradient-to-r from-[#FFAF40] via-[#9A8FEA] to-[#65B4FF] text-white text-sm font-semibold rounded-lg hover:opacity-90 transition-all"
          >
            Get Started
          </Link>
        </div>
  ...
  return (
    <header className="relative pt-24 pb-16 px-4 sm:px-6">
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-gradient-to-b from-[#9A8FEA]/20 via-[#65B4FF]/10 to-transparent rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative max-w-4xl mx-auto text-center">
        ...
        <nav className="flex items-center justify-center gap-2 text-sm text-gray-400 mb-6" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href="/alternative-page-guide" className="hover:text-white transition-colors">Alternative Page Guide</Link>
          {!isPillar && (
            <>
              <span>/</span>
        ...
        {isPillar && (
          <span className="inline-block px-3 py-1 bg-[#9A8FEA]/20 text-[#9A8FEA] text-xs font-semibold rounded-full mb-4">
            COMPREHENSIVE GUIDE
          </span>
        )}
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        {items.map((item, idx) => (
          <div key={idx} className="p-6 bg-white/5 border border-white/10 rounded-xl hover:border-white/20 transition-all hover:-translate-y-1">
            <div className="w-12 h-12 rounded-xl bg-[#9A8FEA]/20 flex items-center justify-center mb-4">
              {item.icon}
            </div>
...
export function ComparisonTable({ title, items }: {
  title: string;
  items: { feature: string; alt: boolean; landing: boolean }[];
}) {
  return (
    <section className="mb-12">
      <h2 className="text-2xl sm:text-3xl font-bold mb-8 text-white">{title}</h2>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-white/10">
              ...
              <th className="py-4 px-4 text-center text-sm font-semibold text-gray-400">Landing Page</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, idx) => (
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {pages.map((page, idx) => (
          ...
          >
            ...
            <span className="inline-flex items-center gap-1 mt-4 text-[#9A8FEA] text-sm font-medium">
              Read more
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function FinalCTA() {
  return (
    <section className="mt-16 p-8 sm:p-12 bg-gradient-to-br from-[#9A8FEA]/20 to-[#65B4FF]/10 rounded-2xl border border-white/10 text-center">
      <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-white">Ready to Create Your Alternative Pages?</h2>
...
export function GuideFooter() {
  return (
    <footer className="border-t border-white/10 mt-20 py-12 px-4 sm:px-6">
