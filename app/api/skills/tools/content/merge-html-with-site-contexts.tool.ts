...
function generateDefaultFooter(siteUrl: string): string {
  ...
  <footer class="bg-gray-50 text-gray-600 py-12 px-6 border-t border-gray-200">
    <div class="max-w-6xl mx-auto">
      <div class="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
        <div class="text-center md:text-left">
          <p class="text-gray-900 font-semibold mb-1">Generated by AI</p>
          <p class="text-xs text-gray-500">Comparison content created with AI assistance</p>
        </div>
        <nav class="flex flex-wrap items-center justify-center gap-4 md:gap-6 text-sm">
          ${baseUrl ? `<a href="${baseUrl}" class="hover:text-gray-900 transition-colors">Home</a>` : ''}
          ${baseUrl ? `<a href="${baseUrl}/privacy" class="hover:text-gray-900 transition-colors">Privacy Policy</a>` : '<a href="/privacy" class="hover:text-gray-900 transition-colors">Privacy Policy</a>'}
          ${baseUrl ? `<a href="${baseUrl}/terms" class="hover:text-gray-900 transition-colors">Terms of Service</a>` : '<a href="/terms" class="hover:text-gray-900 transition-colors">Terms of Service</a>'}
          ${baseUrl ? `<a href="${baseUrl}/contact" class="hover:text-gray-900 transition-colors">Contact</a>` : '<a href="/contact" class="hover:text-gray-900 transition-colors">Contact</a>'}
        </nav>
      </div>
...
- The result is automatically saved to the content item's generated_content field`,
  ...
  execute: async ({ item_id, base_html, header, footer, head_tags }) => {
    try {
      ...
      if (!htmlToProcess && item_id) {
        ...
        if (!headerHtml || !footerHtml) {
          ...
          let contextQuery = getSupabase()
            .from('site_contexts')
            .select('type, content, html, domain_name, seo_project_id')
            .eq('user_id', item.user_id)
            .in('type', ['header', 'footer', 'logo']);
          
          // Use seo_project_id to match site_contexts.seo_project_id
          if (seoProjectId) {
            contextQuery = contextQuery.eq('seo_project_id', seoProjectId);
          } else {
            contextQuery = contextQuery.is('seo_project_id', null);
          }
          
          let { data: contexts, error: contextError } = await contextQuery;
          
          // If no contexts found with seo_project_id, fallback to global contexts (seo_project_id = null)
          ...
          if (contexts && contexts.length > 0) {
            ...
            contexts.forEach((c: any) => {
              console.log(`  - type: ${c.type}, has_html: ${!!c.html}, has_content: ${!!c.content}, seo_project_id: ${c.seo_project_id || 'NULL'}`);
            });
          } else {
            console.log(`[merge_html_with_site_contexts] ⚠️ No site_contexts found! Header and footer will be missing.`);
          }
      ...
      console.log(`[merge_html_with_site_contexts] 🔍 DEBUG INPUT SUMMARY:`);
      ...
      // 🔍 DEBUG: Log header/footer content preview
      if (headerHtml) {
        console.log(`[merge_html_with_site_contexts] 🔍 HEADER PREVIEW (first 500 chars):`);
        console.log(headerHtml.substring(0, 500));
      }
      if (footerHtml) {
        console.log(`[merge_html_with_site_contexts] 🔍 FOOTER PREVIEW (first 500 chars):`);
      ...
      if (bodyMatch) {
        ...
        console.log(`[merge_html_with_site_contexts] 🔍 BODY extracted successfully, length: ${bodyContent.length} chars`);
      } else {
        console.log(`[merge_html_with_site_contexts] ⚠️ WARNING: Could not extract body content from HTML!`);
        console.log(`[merge_html_with_site_contexts] 🔍 HTML starts with: ${htmlToProcess.substring(0, 200)}`);
      }
      ...
      console.log(`[merge_html_with_site_contexts] 🔍 BEFORE MERGE: body length = ${mergedBodyContent.length} chars`);
      console.log(`[merge_html_with_site_contexts] 🔍 BODY starts with: ${mergedBodyContent.substring(0, 200)}`);
      
      // Insert header after opening body tag
      if (headerHtml) {
        console.log(`[merge_html_with_site_contexts] ✅ Inserting header (${headerHtml.length} chars)...`);
        ...
        console.log(`[merge_html_with_site_contexts] ✅ After header insertion: body length = ${mergedBodyContent.length} chars`);
      } else {
        console.log(`[merge_html_with_site_contexts] ⚠️ NO HEADER to insert!`);
      }

      // Insert footer before closing body tag
      if (footerHtml) {
        console.log(`[merge_html_with_site_contexts] ✅ Inserting footer (${footerHtml.length} chars)...`);
        ...
        console.log(`[merge_html_with_site_contexts] ✅ After footer insertion: body length = ${mergedBodyContent.length} chars`);
      } else {
        console.log(`[merge_html_with_site_contexts] ⚠️ NO FOOTER to insert!`);
      }
      
      console.log(`[merge_html_with_site_contexts] 🔍 AFTER MERGE: body length = ${mergedBodyContent.length} chars`);
      console.log(`[merge_html_with_site_contexts] 🔍 MERGED BODY starts with: ${mergedBodyContent.substring(0, 300)}`);
      console.log(`[merge_html_with_site_contexts] 🔍 MERGED BODY ends with: ${mergedBodyContent.substring(mergedBodyContent.length - 300)}`);
...
${mergedBodyContent}
      ...
      if (nullCharCount > 0) {
        console.log(`[merge_html_with_site_contexts] ⚠️ Found ${nullCharCount} null characters (\\u0000), cleaning...`);
        mergedHtml = mergedHtml.replace(/\u0000/g, '');
        console.log(`[merge_html_with_site_contexts] ✅ Null characters cleaned`);
      }
      ...
      if (item_id) {
        ...
        if (saveError) {
          console.error(`[merge_html_with_site_contexts] ❌ SAVE ERROR:`, saveError);
          // 🔧 FIX: Throw error instead of silently continuing - next tool needs this data!
          throw new Error(`Failed to save merged HTML: ${saveError.message}`);
        } else {
          console.log(`[merge_html_with_site_contexts] ✅ SAVE SUCCESS!`);
          // Verify what was saved
          const savedLength = saveResult?.[0]?.generated_content?.length || 0;
          console.log(`[merge_html_with_site_contexts] 🔍 Verified saved content length: ${savedLength} chars`);
          ...
          console.log(`[merge_html_with_site_contexts] 🔍 Header found in saved content: ${hasHeaderInSaved}`);
          console.log(`[merge_html_with_site_contexts] 🔍 Footer found in saved content: ${hasFooterInSaved}`);
        }
      }
      ...
      if (!headerHtml) {
        warnings.push('⚠️ No header found in site_contexts. The page will not have a navigation header. Consider using acquire_site_context tool with field="header" or configure it in Brand & Context.');
      }
      if (!footerHtml) {
        warnings.push('⚠️ No footer found in site_contexts. Consider using acquire_site_context tool with field="footer" or configure it in Brand & Context.');
      }
