...
- Get header/footer/head_tags from get_header, get_footer, get_head_tags tools
- The result is automatically saved to the content item's generated_content field`,
  parameters: z.object({
    ...
    header: z.string().optional().describe('HTML content for the site header/navigation (will be inserted after <body>)'),
    footer: z.string().optional().describe('HTML content for the site footer (will be inserted before </body>)'),
  ...
  execute: async ({ item_id, base_html, header, footer, head_tags }) => {
    try {
      ...
      if (header) {
        ...
        mergedBodyContent = leadingWhitespace + header.trim() + '\n' + mergedBodyContent.replace(/^\s*/, '');
      }
...
${mergedBodyContent}
      ...
      return {
        ...
        message: `HTML merged successfully and saved to database. The next step (fix style conflicts) will read the content from the database using the item_id.`,
      };
    } catch (error: any) {
